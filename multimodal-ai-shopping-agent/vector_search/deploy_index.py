...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(str(Path(__file__).parent.parent / 'logs' / 'index_deployment.log'))
    ]
)
...
def main():
    ...
    try:
        ...
        logger.info("")
        logger.info("Step 2: Finding or creating endpoint...")
        endpoint = manager.get_or_create_endpoint(settings.VS_ENDPOINT_NAME)
        logger.info(f"✓ Endpoint ready: {endpoint.display_name}")
