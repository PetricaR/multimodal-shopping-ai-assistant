...
if [ -n "$BACKEND_URL" ]; then
  ...
  # Update frontend .env with backend URL only - API key is fetched at runtime from backend
  echo "📝 Updating frontend environment..."
  cat > ./app/.env.production <<EOF
VITE_API_URL=${BACKEND_URL}
VITE_API_BASE_URL=${BACKEND_URL}/api/v1
EOF
  echo "✅ Environment configured (API key served at runtime, not baked into bundle)"
else
