...
class JamilaRecipeScraper:
    ...
    def _select_best_recipe_with_ai(self, user_query: str, search_results: List[Dict[str, str]]) -> Optional[Dict[str, str]]:
        ...
        try:
            ...
            api_key_str = get_gemini_api_key()
            if not api_key_str:
    ...
    def _extract_ingredients(self, soup: BeautifulSoup) -> List[str]:
        ...
        if not ingredients:
            ...
            for selector in selectors:
                ...
                if elements:
                    # Validate if this looks like an ingredient list (heuristic)
                    # Check if first item has numbers or common ingredient words if using generic selector
                    if 'wp-block-list' in selector:
                        # Be conservative with generic lists: Check context or content
                        # Or better yet, rely on Header search below which is safer
                        continue 
        ...
        # 2. Header-based search (Robust fallback for Gutenberg/Custom layouts)
        if not ingredients:
            headers = soup.find_all(['h2', 'h3', 'h4', 'h5', 'p', 'strong'])
            for header in headers:
                header_text = header.get_text(strip=True).lower()
                if 'ingrediente' in header_text and len(header_text) < 30:
                    # Found standard header, look for next list
                    next_elem = header.find_next_sibling()
                    # Traverse a few siblings to find a list (sometimes there are spacers)
                    siblings_checked = 0
                    while next_elem and siblings_checked < 3:
                        ...
                        elif next_elem.name == 'div' and next_elem.find('ul'):
                             # Nested list
                            items = next_elem.find('ul').find_all('li')
                            for item in items:
                                text = item.get_text(strip=True)
                                if text:
                                     text = re.sub(r'^[▢\s\-•]+', '', text).strip()
