...
async def extract_product_data(session, base_url, category_name, store, fetch_details=False, max_products=None, semaphore=None):
    """
        ...
        try:
            async with session.get(current_url) as response:
                if response.status == 200:
                    ...
                    logger.info(f"Page {page}: Extracted {len(page_products_list)} products. Total so far: {len(all_products_list)}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Bringing Products Scraper')
    ...
    parser.add_argument('--max-categories', type=int, default=30, help='Limit number of categories to scrape (for testing)')
    parser.add_argument('--max-products', type=int, default=100, help='Limit products per category (for testing)')
    parser.add_argument('--no-details', action='store_true', help='Disable detailed product info fetching (faster)')
    
    parser.add_argument('--category', type=str, help='Filter to specific category (name or slug component)')
